...
/* --------------------------------------------------------------------
   3. NAV — mobile toggle, scroll spy, backdrop state
-------------------------------------------------------------------- */
function initNav() {
  ...
  const spy = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        navLinkEls.forEach(a => a.classList.toggle('active', a.getAttribute('href') === '#' + entry.target.id));
      }
    });
...
function initWatchlistSection() {
  ...
  async function render() {
    ...
    wrap.innerHTML = `<table class="data-table"><thead><tr><th>Symbol</th><th>Type</th><th>Price</th><th>24h</th><th></th></tr></thead><tbody>${
